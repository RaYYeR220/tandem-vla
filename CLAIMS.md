...
| A hand-off is structurally required, not decorative | **REPRODUCIBLE** | `tandem/sim/layout.py`: the staging region and the place-setting slots have no overlap in either arm's envelope; `scripts/eval_gate.py` shows the gate inserting the hand-off |
